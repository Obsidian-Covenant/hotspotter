...
int main (int argc, char *argv[])
{
    ...
    {
        // Remove newlines
        size_t l = strlen(ssidlist[count]);
        if (l > 0 && ssidlist[count][l-1] == '\n')
            ssidlist[count][l-1] = '\0';
        count++;
    }
    ...
    while (1)  
    {
        ...
        if (tagtype == E_SSID)
        {
            ...
            memcpy(essid, packet + offset, taglen);
            essid[taglen] = '\0';
            ...
            if (pre_apexecute == 1)
            {
                ...
                pre_execute);
                int rc = system(pre_execute);
                if (rc == -1) {
                    perror("system(pre_execute)");
                } else if (WIFEXITED(rc)) {
                    int code = WEXITSTATUS(rc);
                    if (code != 0) {
                        fprintf(stderr, "pre_execute exited with code %d\n", code);
                    }
                } else if (WIFSIGNALED(rc)) {
                    fprintf(stderr, "pre_execute killed by signal %d\n", WTERMSIG(rc));
                } else {
                    fprintf(stderr, "pre_execute ended unexpectedly (status=0x%x)\n", rc);
                }
            }
            ...
            if (post_apexecute == 1)
            {
                ...
                post_execute);
                int rc = system(post_execute);
                if (rc == -1) {
                    perror("system(post_execute)");
                } else if (WIFEXITED(rc)) {
                    int code = WEXITSTATUS(rc);
                    if (code != 0) {
                        fprintf(stderr, "post_execute exited with code %d\n", code);
                    }
                } else if (WIFSIGNALED(rc)) {
                    fprintf(stderr, "post_execute killed by signal %d\n", WTERMSIG(rc));
                } else {
                    fprintf(stderr, "post_execute ended unexpectedly (status=0x%x)\n", rc);
                }
            }
...
int handle_ap(char *ssid, struct iwreq iwr, int socketfd, int automatic,
	...
	else
	{
		...
		fflush (stdout);
        if (fgets(becomeapbuf, sizeof(becomeapbuf), stdin) == NULL) {
...
int handle_ssid(char *ssid, char *lastssid) {
...
int become_ap(char *essid, int socketfd, char *interface) {
	...
	strncpy(iwr.ifr_name, interface, IFNAMSIZ - 1);
